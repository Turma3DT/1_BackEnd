...
export class StatusController {
    public static post = (req: Request, res: Response) => {
        try {
            const { status, velocidade, perdido } = req.body;

            console.log(req.body);

            if (velocidade === undefined || status === undefined || perdido === undefined) {
                throw new ExceptionAPI('400');
            }

            if (typeof status !== 'boolean' || typeof velocidade !== 'string' || typeof perdido !== 'boolean' ) {
                throw new ExceptionAPI('406');
            }

            const sucesso = StatusRepositorie.salvarStatus({ status, velocidade, perdido });
